...
function convertToRomanNumerals(num) {
  ...
  const tens = (n, ten) => (n > 1 ? `X${tens(n - 1, ten)}` : 'X');
  const x = Math.floor(num / 10);
  const y = `${num % 10}`;
  return num > 10 ? `${tens(x, '')}${roman[y]}` : `${roman[y]}`;
}
...
function isContainNumber(num, digit) {
  ...
  return t > 0 ? isContainNumber(t, digit) : false;
}
...
function rotateMatrix(matrix) {
  ...
  for (let j = 0; j < Math.ceil(matrix.length / 2); j += 1) {
    for (let i = 0; i <= l; i += 1) {
      newMatrix[y + i][l] = matrix[y][x + i];
      newMatrix[l][l - i] = matrix[y + i][l];
      newMatrix[l - i][x] = matrix[l][l - i];
      newMatrix[y][x + i] = matrix[l - i][x];
    }
    x += 1;
    y += 1;
    l -= 1;
...
function sortByAsc(arr) {
  ...
  const result = [...sortByAsc(left), ...equal, ...sortByAsc(right)];
  for (let i = 0; i < result.length; i += 1) {
    shadowArr[i] = result[i];
  }
  return shadowArr;
}
...
function shuffleChar(str, iterations) {
  ...
  for (let j = 1; j <= iterations; j += 1) {
    ...
    res = `${even}${odd}`;
    if (res === str) return shuffleChar(str, iterations % j);
  }
...
function getNearestBigger(number) {
  const nums = Array.from(`${number}`, (e) => Number(e));
  const rest = [];

  function isReplaced(i) {
    for (let j = 0; j < rest.length; j += 1) {
      if (rest[j] > nums[i - 1]) {
        [rest[j], nums[i - 1]] = [nums[i - 1], rest[j]];
        return true;
      }
    }
    return false;
  ...
  for (let i = nums.length - 1; i > 0; i -= 1) {
    ...
    if (isReplaced(i)) {
      for (let j = 0; j < rest.length; j += 1) {
        nums[nums.length - 1 - j] = rest[rest.length - 1 - j];
      }
      break;
    }
  }
